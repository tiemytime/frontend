...
const PersistentStarfield = () => {
  const canvasRef = useRef(null);
  const starsRef = useRef([]);
  const animationFrameRef = useRef(null);
  const { isVisible, animationSpeed, particleCount } = useBackgroundStore();
  ...
  useEffect(() => {
    ...
    const createStars = (width, height) => {
      ...
      // Use dynamic star count from store
      const baseStarCount = Math.floor((width * height) / 2200) + 1600;
      const starCount = Math.floor(baseStarCount * (particleCount / 200));
    ...
    const drawStar = (ctx, star, time) => {
      // Gentle twinkling effect with speed control
      const currentOpacity = star.baseOpacity * (Math.sin(time * star.twinkleSpeed * 0.0008 * animationSpeed) * 0.2 + 0.8);
  ...
  }, [isVisible, animationSpeed, particleCount]);
...
export default React.memo(PersistentStarfield);
